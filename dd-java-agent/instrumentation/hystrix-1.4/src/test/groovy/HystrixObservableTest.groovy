...
class HystrixObservableTest extends AgentTestRunner {
  static {
    // Increase so fallback test doesn't trip the breaker.
    System.setProperty("hystrix.command.default.circuitBreaker.requestVolumeThreshold", "100")
  ...
  def "test no fallback results in error for #action"() {
    ...
    runUnderTrace("parent") {
      def val = operation new HystrixObservableCommand<String>(asKey("FailingGroup")) {
